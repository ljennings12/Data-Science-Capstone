...
tibble(
  actual = train$over,
  train_pred = predict(nfl_NB, newdata = train),
  pred = case_when(
    train_pred == 1 ~ 1,
    train_pred == 0 ~ 0
  )
...
## fit model
nfl_NB_points <- naiveBayes(
  # formula
  over ~
    net_ppg + net_ppg_allowed,
...
tibble(
  actual = train$over,
  train_pred = predict(nfl_NB_points, newdata = train),
  pred = case_when(
    train_pred == 1 ~ 1,
    train_pred == 0 ~ 0
  )
...
tibble(
  actual = test$over,
  train_pred = predict(nfl_NB_points, newdata = test),
  pred = case_when(
    train_pred == 1 ~ 1,
    train_pred == 0 ~ 0
  )
